...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _startEdit(User user) {
    // LỖI 2 (Null check operator): dùng `!` trên giá trị chưa bao giờ được đặt.
    final String? draft = _lastDraft;
    debugPrint('Bản nháp: ${draft!.length} ký tự');
    setState(() {
      _editingId = user.id;
  ...
  void _clearAll() {
    // LỖI 3 (Bad state: No element): tìm người dùng theo điều kiện không ai thoả.
    final User admin = _repository.users.firstWhere((User u) => u.email == 'admin@fpt.edu.vn');
    debugPrint('Giữ lại ${admin.fullName}');
    setState(_repository.clear);
  }

  void _confirmDelete(User user) {
    // LỖI 1 (RangeError): dùng length làm chỉ số phần tử cuối — lệch một đơn vị.
    final User last = _repository.users[_repository.users.length];
    debugPrint('Người cuối danh sách: ${last.fullName}');
    showDialog<void>(
      context: context,
