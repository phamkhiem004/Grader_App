...
| **E2E** | Chạy thật trọn đường: soạn đề UI → publish → chấm Docker → result.json | ✅ **Xong** 2026-08-08 | Đề `PE_E2E_01` 5 testcase, chấm bài `high` = 6.0, **0 luật FAIL**; 6 cổng chặn đỏ đúng trên HTTP thật; matrix máy sinh có đủ 3 hình rubric (nhãn 2 thành viên · nhãn 1 thành viên · cụm gộp 40=20+20); UI xác nhận sống: ô requirements + bộ đếm, 2 nút nhóm đúng ngưỡng 1/2, 2 modal đúng chữ. Phát mẫu thứ 9 `latest-authored-exam.json` — mẫu ĐẦU TIÊN không từ fixture. Còn nợ: 5 dòng fixture khai `targetType` ngoài schema ⇒ fixture vẫn chưa sản xuất được trọn; FE có race khi bấm "Thêm vào đề" hai lần trong cùng một tick (trùng `instance_id`) |
